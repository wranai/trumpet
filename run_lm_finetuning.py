...
class TextDataset(Dataset):
    def __init__(self, tokenizer, file_path='train', block_size=512, map = False):
        ...
        if os.path.exists(cached_features_file) and self.cached_features_state > 0:
            ...
            for i, line in enumerate(open(file_path, encoding="utf-8").readlines()):
                ...
                self.cached_features[self.cached_features_state:][:length] = torch.tensor(tokenized_text, dtype = self.cached_features.dtype, device = 'cpu')
                if i % 10000 == 0: print(i, line, tokenized_text)
                self.cached_features_state[None] += length
    ...
    def __getitem__(self, item):
        return self.examples[item].long()
